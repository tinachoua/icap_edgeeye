	...
	try
	{
		...
		if (authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			...
			else if (IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				...
				//send the response
				if (strstr(RecvBuf, "charset")!=NULL) snprintf(SendBuf, BUFFER_SIZE, "charset=utf-8,username=\"%s\"", m_sLogin.c_str());
...
void CSmtp::FormatHeader(char* header)
{
	...
	snprintf(header, BUFFER_SIZE, "Date: %s, %d %s %d %d:%d:%d +0800\r\n", 

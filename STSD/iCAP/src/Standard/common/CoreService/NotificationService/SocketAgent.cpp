					...
					if (i == sock_fd) {
                        ...
                        if (recv_len == -1) {
							...
							if (write_len != sizeof(PACKAGE)) {
								...
								if (result == CMD_UPDATE_TH)
									event_handler.GetTHSetting();
								else if (result == CMD_UPDATE_SMTP)
									event_handler.GetMailSendor();
							}
						}
...
int CheckPackage(PACKAGE* input_buffer)
{
    ...
    if ((length != input_buffer->Length) || (strcmp(checkall, input_buffer->CheckSum) != 0)) {
		LOG_WARN << "Wrong package, drop...";
        return -1;
	}

	if ((strcmp(input_buffer->Header, "INNO") != 0) || (strcmp(input_buffer->Command, "N") != 0)) {
		LOG_WARN << "Wrong header or command, drop...";
        return -1;
	}

	if (strcmp(input_buffer->Payload, "TH/UPDATE") == 0) {
		LOG_INFO << "Get the threshold changed package.";
		return CMD_UPDATE_TH;
	} else if (strcmp(input_buffer->Payload, "SMTP/UPDATE") == 0) {
		LOG_INFO << "Get the mail server changed package.";
		return CMD_UPDATE_SMTP;
	} else {
		LOG_WARN << "Wrong payload, drop...";
        return -1;
	}
...
PACKAGE* GenerateStatusPackage(int status)
{
    ...
    strncpy(spackage->Command, "N", sizeof(spackage->Command));

...
export function insertSpecialTable(id) {
    return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:0px;" width="100%">' +
        ...
        '<br>' +
        '<button ' + 'id =send-' + id + ' name = "send-message" style = "margin-top:116px" >Send</button>' + '&nbsp&nbsp&nbsp&nbsp' + `<span id="send-result-${id}"></span>` + '</td>' +
        '</tr>';
}
...
export function getSuccess(response, widgetId, labelIndex) {
    ...
    $('#modal-data tbody').on('click', 'td.device-link-control button', (e) => {
        ...
        promise.GetDetail = APICaller.GET(`DeviceInfoAPI/GetDetail?DeviceName=${targetDevice}`, null, null, false);
        promise.GetOverView = APICaller.GET(`DeviceInfoAPI/GetOverview?DeviceName=${targetDevice}`, null, null , false);
        promise.GetLocation = APICaller.GET(`DeviceInfoAPI/GetLocation?DeviceName=${targetDevice}`, false);
        promise.GetImg = APICaller.GET('DeviceAPI/GetImg', 'devName', targetDevice, false);

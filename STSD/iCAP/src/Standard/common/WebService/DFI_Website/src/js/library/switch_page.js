        ...
        var initPage = function (deviceData) {
            ...
            if (clickFlag.initTable === false) {
                let initDeviceTable = function () {
                    var $Attach = $('<thead style = "background-color: rgb(143,188,183);"><tr></tr></thead>');
                ...
                let addDeviceInfoTableData = function (deviceData) {
                    ...
                    var td = `<td>${specificDevice[0].ownerName}</td>` +
                        '<td>' + suffer + '</td>' +
                        `<td><i id = 'send-message' class="fa fa-envelope-o" aria-hidden="true" style="cursor: pointer;"><font face="Arial">Send Message</font></i></td>`;

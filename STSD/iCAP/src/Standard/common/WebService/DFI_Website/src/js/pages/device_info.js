...
import { makeColorsWithoutRed, getWarningColor } from "../library/color";
...
import '../../style/DFI/device_info.css';
import '../../style/DFI/EAPI_info.css';
...
import IMG_INNOAGE from '../../assets/innoAGE.svg';
    ...
    $.ajax({
      ...
      success: function (response) {
        function RenderStorageAnalysisData(data) {
          var storName = [];
          var storData = [];
          var chartColor = makeColorsWithoutRed(data.Lifespan.length);
    ...
    $.ajax({
      ...
      success: function (response) {
            ...
            function SwitchTo(pageTag) {
              ...
              for (var i = 0; i < tabs.length; i++) {
                  ...
                  let RenderDeviceMAP = function (parsed_data) {
                    ...
                    parsed_data.mapList.forEach((element, idx) => {
                      ...
                      if (parsed_data.mapId === element.Id) {
                        ...
                        buttonElement.style.background = 'rgb(109,153,152)';
                      }
                      ...
                      buttonElement.addEventListener('mousedown', (event) => {
                        var removeClick = function (button) {
                          ...
                          buttonArray.some((item) => {
                            if (item.classList.contains('button-click')) {
                              ...
                              item.style.color = 'rgb(109,153,152)';
                        ...
                        event.target.style.background = 'rgb(109,153,152)';
                ...
                if (pageTag === "OOB") {
                  ...
                  const ageLogo = document.getElementById('InnoAGE-logo');

                  ageLogo.src = IMG_INNOAGE;
                  oobService.appendChild(oobDirector.element);
                  oobDirector.setupService({
                    activeServices: ['power', 'reboot', 'recovery']
                  });
                  tableBody.innerHTML = '';
                  createIndexBtn({
                    ooblist,
                    onclick: async function (i) {
                      oobDirector.setTargetSN(ooblist[i].SN);
                      tableBody.innerHTML = '';
                      deviceManager.getOOBInfo({ devName, sn: ooblist[i].SN }).then((ismart) => {
                        // Get and show SSD info 
                        tableBody.appendChild(createBody(ismart, ooblist[i].SN));
                      }).catch(() => {
                        console.log('getOOBInfo API error');
                        return;
                      });
                    }
                  })
                ...
                if (pageTag === "EAPI") {
                  const tempTds = document.querySelectorAll('#temp-table tr > td + td');
                  const fanTds = document.querySelectorAll('#fan-table tr > td + td');
                  const voltageTds = document.querySelectorAll('#voltage-table tr td:nth-child(2n)');
                  ...
                  $.ajax({
                    ...
                    if (respnse) {
                      const {
                        ...
                        FAN: {
                          CPUFAN, SYS2FAN, SYSFAN
                        },
                        VOLTAGE
                      } = respnse;

                      tempTds[0].innerHTML = `
                                      <div>${isNaN(CPUTEMP) ? 'N/A' : CPUTEMP.toFixed(2) + '℃'}</div>
                                      <div>${isNaN(CPUTEMP) ? 'N/A' : (CPUTEMP * 9 / 5 + 32).toFixed(2)}</div>
                                    `;
                      tempTds[1].innerHTML = `
                                      <div>${isNaN(SYSTEMP) ? 'N/A' : SYSTEMP.toFixed(2) + '℃'}</div>
                                      <div>${isNaN(SYSTEMP) ? 'N/A' : (SYSTEMP * 9 / 5 + 32).toFixed(2) + '˚F'}</div>
                                    `;

                      fanTds[0].innerHTML = isNaN(CPUFAN) ? 'N/A' : `${CPUFAN} RPM`;
                      fanTds[1].innerHTML = isNaN(SYSFAN) ? 'N/A' : `${SYSFAN} RPM`;
                      fanTds[2].innerHTML = isNaN(SYS2FAN) ? 'N/A' : `${SYS2FAN} RPM`;

                      voltageTds[0].innerHTML = isNaN(VOLTAGE['VCORE']) ? 'N/A' : `${VOLTAGE['VCORE'].toFixed(3)} V`;
                      voltageTds[1].innerHTML = isNaN(VOLTAGE['+12V']) ? 'N/A' : `${VOLTAGE['+12V'].toFixed(3)} V`;
                      voltageTds[2].innerHTML = isNaN(VOLTAGE['VBAT']) ? 'N/A' : `${VOLTAGE['VBAT'].toFixed(3)} V`;
                      voltageTds[3].innerHTML = isNaN(VOLTAGE['3VSB']) ? 'N/A' : `${VOLTAGE['3VSB'].toFixed(3)} V`;
                      voltageTds[4].innerHTML = isNaN(VOLTAGE['5V']) ? 'N/A' : `${VOLTAGE['5V'].toFixed(3)} V`;
                      voltageTds[5].innerHTML = isNaN(VOLTAGE['VDDQ']) ? 'N/A' : `${VOLTAGE['VDDQ'].toFixed(3)} V`;
                    }
                  }).catch(() => {
                    ...
                    function makeTempBar(labels, TempAnalysis) {
                      var data = {
                        ...
                        datasets: [{
                          label: 'probability',
                          data: TempAnalysis,
                          backgroundColor: 'rgb(24,149,157)',
                          borderWidth: 1
                        }],
                      }
                      var options = {
        ...
        function RenderStorageInfo(parsed_data) {
          ...
          const result = parsed_data.Stor.sort((a, b) => {
        ...
        function RenderGPUInfo(parsed_data) {
          ...
          if (parsed_data.GPUDynamic) {
            ...
            // if (typeof (GPUDynamicData.Temp) === 'string') {
            //     $('#gpuTemperature').html(GPUDynamicData.Temp.split('C', 1));
            // } else {
            //     $('#gpuTemperature').html(GPUDynamicData.Temp);
            // }

            // if (typeof (GPUDynamicData.FanTemp) === 'string') {
            //     $('#gpuFanTemperature').html(GPUDynamicData.FanTemp.split('C', 1));
            // } else {
            //     $('#gpuFanTemperature').html(GPUDynamicData.FanTemp);
            // }

            // if (typeof (GPUDynamicData.MemUsed) === 'sting') {
            //     $('#gpuMemoryUsed').html(GPUDynamicData.MemUsed.split('M', 1));
            // } else {
            //     $('#gpuMemoryUsed').html(GPUDynamicData.MemUsed);
            // }

            //CPU each core ( # : 0 ~ 5 ) test data
            // let coreFreq = [1454, 1355, 1120, 1011, 1497, 1543];
            // let coreLoading = [11, 44, 77, 88, 55, 66];
            // let coreStatus = [true, true, false, true, true, false];

            //gpu core info
            // let gpuTabCoreFreq = [];
            // let gpuTabCoreLoading = [];
            // let gpuTabCoreStatus = [];

            // gpuTabCoreFreq.push(`<span style="font-size: 12px;color: #888;">Frequency -</span>`);
            // gpuTabCoreLoading.push(`<span style="font-size: 12px;color: #888;">Loading -</span>`);
            // gpuTabCoreStatus.push(`<span style="font-size: 12px;color: #888;">Status -</span>`);

            // for (let i = 0; i < coreFreq.length; i++) {
            //     gpuTabCoreFreq.push(`<div style="padding-top: 5px;">
            // <div style="float:left;line-height: 40px;font-size: 12px;">Core #${i}</div>
            // <div class="text-right">
            // <span class="text-right" style="font-weight: 700;height: 40px;line-height: 38px;font-size: 40px;">${coreFreq[i]}</span>
            // <span class="fan-unit">MHz</span></div>
            // </div>`);

            //     gpuTabCoreLoading.push(`<div style="padding-top: 5px;">
            // <div style="float:left;line-height: 40px;font-size: 12px;">Core #${i}</div>
            // <div class="text-right"><span class="text-right" style="font-weight: 700;height: 40px;line-height: 38px;font-size: 40px;">${coreLoading[i]}</span>
            // <span class="fan-unit">%</span></div></div>`);

            //     gpuTabCoreStatus.push(`<div style="padding-top: 5px;">
            // <div style="float:left;line-height: 40px;font-size: 12px;">Core #${i}</div>
            // <div class="text-right">
            // <span id="fan-value" class="text-right" style="font-weight: 700;height: 40px;line-height: 38px;font-size: 40px;"></span>
            // <span class="${coreStatus[i] ? 'led-success' : 'led-danger'}"></span></div></div>`);
            // }

            // $('#gpu-tab-core-freq').html(gpuTabCoreFreq.join(''));
            // $('#gpu-tab-core-loading').html(gpuTabCoreLoading.join(''));
            // $('#gpu-tab-core-status').html(gpuTabCoreStatus.join(''));
          }
        }
        ...
        if (parsed_data.EXT && parsed_data.EXT.length !== 0) {
          ...
          sub_tabs = [
            "Overview", "OS", "CPU", "MB", "MEM", "EAPI", "Storage", "NET", "External", "Location"
          ];
          ...
          sub_tabs = [
            "Overview", "OS", "CPU", "MB", "MEM", "EAPI", "Storage", "NET", "Location"
          ];
        }
        ...
        if (parsed_data.isEAPI) {
          sub_tabs.push('EAPI');
          sub_tabs_tooltip.push(null);
        }
